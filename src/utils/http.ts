...
import { isAxiosUnauthorizedError, isExpiredTokenError } from './utils'
...
class Http {
  ...
  constructor() {
    ...
    this.instance.interceptors.response.use(
      ...
      (error: AxiosError) => {
        //Chỉ toash lỗi không phải 422 và 401
        if (
        ...
        if (isAxiosUnauthorizedError<ErrorResponse<{ name: string; message: string }>>(error)) {
          ...
          // Trường hợp token hết hạn và request đó không phải là request refresh token thì chúng ta mới tiến hành gọi refresh token
          if (isExpiredTokenError(error) && url !== URL_REFRESH_TOKEN) {
            ...
            return this.refreshTokenRequest.then((access_token) => {
              // Nghĩa là chúng ta gọi lại request cũ vừa bị lỗi
          ...
          // Không truyền token
          // thì tiên hành clear local storage và toast mesage
          clearLS()
          this.accessToken = ''
          this.refreshToken = ''
